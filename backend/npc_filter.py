...
class NPCFilter:
    ...
    async def filter_npcs_by_relevance(
        ...
        # 使用LLM筛选
        return await self._llm_filter_npcs(player_input, player_action, available_npcs, max_npcs)
    ...
    async def _llm_filter_npcs(
        ...
        system_prompt = f"""
        你是一个COC跑团的NPC筛选器。根据玩家的行动和当前情况，从可用NPC中选择最相关的{max_npcs}个NPC来做出反应。
        ...
        筛选标准:
        1. **直接相关性**: NPC是否与玩家行动直接相关（如：玩家询问信息，NPC知道相关内容）
        2. **位置相关性**: NPC是否在玩家行动发生的位置附近
        3. **状态相关性**: NPC当前状态是否允许或需要做出反应
        4. **剧情相关性**: NPC是否与当前剧情发展相关
        5. **角色重要性**: 重要NPC优先考虑
